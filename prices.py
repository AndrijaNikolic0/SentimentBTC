...
while i<15:
    ...
    if i==0:
        df.to_csv('coinbase1.csv', index=False)
    else:
        df.columns = df.iloc[0]
        df = df[1:]
        df.to_csv('coinbase1.csv', mode='a', index=False)
...
while i<15:
    ...
    if i==0:
        df.to_csv('coinbase2.csv', index=False)
    else:
        df.columns = df.iloc[0]
        df = df[1:]
        df.to_csv('coinbase2.csv', mode='a', index=False)
...
final.columns = ['time', 'open_btc', 'high_btc', 'low_btc','close_btc'
                  , 'open_sol', 'high_sol', 'low_sol','close_sol',
                  'open_doge', 'high_doge', 'low_doge','close_doge']
